...
int _replace_vars(info_t *info)
{
    ...
    for (i = 0; info->argv[i]; i++)
    {
        ...
        {
            _replace_string(&(info->argv[i]), _strdup(_convert_number(info->status, 10, 0)));
            continue;
        }
        if (!_strcmp(info->argv[i], "$$"))
        {
            _replace_string(&(info->argv[i]), _strdup(_convert_number(getpid(), 10, 0)));
            continue;
        }
